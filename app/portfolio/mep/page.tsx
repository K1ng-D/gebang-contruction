...
// ===== Data (khusus MEP) =====
const highlights = [
  { icon: FiShield, title: "Compliance & Bergaransi", desc: "Pemasangan mengikuti SNI/PUIL & rekomendasi pabrikan." },
  { icon: FiClock, title: "On Schedule", desc: "Shop drawing jelas, koordinasi lintas disiplin rapi." },
  { icon: FiTool, title: "Material & Tools Standar", desc: "MCB/MCCB, kabel SNI, pipa HDPE/PPR, alat ukur terkalibrasi." },
  { icon: FiMapPin, title: "Solo Raya", desc: "Solo, Sukoharjo, Karanganyar, Boyolali & sekitar." },
];

const scopes = [
  // Mechanical (HVAC & Fire)
  "HVAC: AC split/VRF, ducting, diffuser, balancing airflow",
  "Ventilasi & exhaust (toilet/dapur/gudang), louver & damper",
  "Fire safety: hydrant, APAR layout, detektor asap/panas (rancang rute & pemasangan)",
  // Electrical
  "Panel listrik (SDP/MDP/Sub), MCB/MCCB, ELCB/RCD",
  "Distribusi: tray/ladder/conduit, kabel SNI, labeling & as-built",
  "Penerangan: general/ambient/emergency lighting + photometric basic",
  "Sistem pembumian (earthing), penangkal petir (LA), SPD",
  "Backup: genset ATS/AMF, UPS, opsi solar PV (prarencana)",
  "Low voltage & IT: CCTV, access control, LAN/FO basic routing",
  // Plumbing
  "Plumbing air bersih: PPR/HDPE, pompa, toren, PRV/NRV",
  "Air kotor & grey water: uPVC, vent, grease trap, manhole",
  "Sanitair: set WC, floor drain, floor trap, waterproofing area basah",
  "Testing & commissioning: megger, continuity, pressure test",
  "Dokumentasi: BOQ/RAB, shop drawing, as-built drawing, manual O&M",
];

// Testimoni
type Testi = { name: string; role: string; comment: string; rating: number };
const testimonials: Testi[] = [
  { name: "PT Sumber Jaya", role: "Gudang – Palur", comment: "Panel & jalur kabel rapi, label jelas, commissioning lancar.", rating: 5 },
  { name: "Klinik Sehat", role: "Bangunan – Solo", comment: "Plumbing PPR bersih, pressure test aman. Air panas & dingin stabil.", rating: 5 },
  { name: "Ruko Harmoni", role: "Komersial – Jebres", comment: "AC & exhaust balance, ruangan nyaman. Emergency lamp terintegrasi.", rating: 5 },
];

// Proyek MEP
const projectTeaser = [
  { id: "MEP-001", title: "Upgrade Panel & Lighting Emergency", location: "Solo", year: 2025, cover: project1.src, tags: ["Panel", "Emergency"] },
  { id: "MEP-002", title: "Instalasi HVAC & Ducting – Office", location: "Colomadu", year: 2024, cover: project2.src, tags: ["HVAC", "Ducting"] },
  { id: "MEP-003", title: "Plumbing PPR + Grease Trap – Resto", location: "Kartasura", year: 2024, cover: project3.src, tags: ["Plumbing", "Kitchen"] },
  { id: "MEP-004", title: "CCTV & Access Control – Ruko", location: "Sukoharjo", year: 2025, cover: project4.src, tags: ["CCTV", "Access"] },
];
...
export default function MEPPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-blue-50 to-white text-slate-800">
      <Head>
        <title>MEP (Mechanical, Electrical, Plumbing) | Gebang Construction</title>
        <meta
          name="description"
          content="Jasa MEP lengkap: HVAC, panel & distribusi listrik, grounding & petir, plumbing air bersih/kotor, fire safety, CCTV & access. Sesuai SNI/PUIL, bergaransi."
        />
      </Head>
          ...
          <div className="grid lg:grid-cols-2 gap-10 py-16 lg:py-24 items-center">
            <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} className="space-y-6">
              <motion.h1 variants={fadeUp()} className="text-4xl/tight md:text-5xl/tight font-bold tracking-tight">
                Layanan <span className="bg-gradient-to-r from-amber-400 to-amber-300 bg-clip-text text-transparent">MEP</span> Terintegrasi
              </motion.h1>
              <motion.p variants={fadeUp(0.1)} className="text-blue-100 max-w-xl">
                Perencanaan, instalasi, sampai commissioning—HVAC, listrik, plumbing, dan fire safety yang aman, efisien, dan rapi dokumentasinya.
              </motion.p>
              ...
              <motion.ul variants={fadeUp(0.3)} className="flex items-center gap-4 text-blue-100 text-sm">
                <li className="inline-flex items-center gap-2"><FiCheckCircle className="text-amber-400" /> Compliance SNI/PUIL</li>
            ...
            <motion.div className="relative" variants={prefersReducedMotion ? undefined : float} initial="initial" animate="animate">
              <div className="aspect-[4/3] rounded-3xl bg-cover bg-center shadow-2xl relative overflow-hidden">
                ...
                <img src={heroImage.src} alt="MEP – Gebang Construction" className="w-full h-full object-cover" />
              </div>
      ...
      <section id="keunggulan" className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Koordinasi lintas disiplin untuk meminimalkan rework & percepat serah-terima.</p>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Bisa paket lengkap (design & build) atau sebagian sesuai kebutuhan.</p>
        </div>
      ...
      <section id="proyek" className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12 flex items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-blue-900">Proyek MEP Terbaru</h2>
            <p className="text-slate-600 mt-2 max-w-2xl">Beberapa pekerjaan MEP terakhir—HVAC, panel & plumbing dengan dokumentasi lengkap.</p>
          </div>
          <Link
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Install rapi, aman, dan terdokumentasi—siap audit & maintenance.</p>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Butuh asistensi perhitungan beban, single line, atau gambar plumbing? Kita bantu.</p>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
          {[
            { q: "Dokumen apa yang disiapkan?", a: "RAB/BOQ, shop drawing (single line/risers), metode kerja, test reports (megger/continuity/pressure), dan as-built." },
            { q: "Apakah bisa partial pekerjaan?", a: "Bisa. Misal hanya panel & distribusi listrik, atau hanya plumbing PPR/HDPE." },
            { q: "Bagaimana standard safety saat instalasi?", a: "LOTO, APD lengkap, uji kebocoran/insulasi, serta pengujian sebelum energize." },
            { q: "Skema pembayaran?", a: "Termin progres (DP → mid → serah terima). Semua transparan & terdokumentasi." },
          ].map((f, idx) => {
      ...
      <section id="kontak" className="relative mx-auto max-w-7xl px-4 pb-16">
        <div className="rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-900 to-blue-700 p-8 md:p-12 shadow-sm overflow-hidden text-white">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <h3 className="text-2xl md:text-3xl font-bold tracking-tight">Konsultasi MEP & Estimasi Gratis</h3>
              <p className="text-blue-100 mt-2 max-w-prose">
                Kirimkan denah arsitektur, kebutuhan beban/kapasitas, dan foto lokasi—kami bantu konsep, RAB, serta jadwal eksekusi.
              </p>
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                <a
                  href="mailto:hello@gebangconstruction.id?subject=Konsultasi%20MEP"
            ...
            <motion.div className="aspect-[4/3] rounded-2xl bg-cover bg-center shadow-lg overflow-hidden" variants={prefersReducedMotion ? undefined : float} initial="initial" animate="animate">
              <img src={ctaImage.src} alt="Konsultasi MEP" className="w-full h-full object-cover" />
            </motion.div>
          </div>
        </div>
      </section>
