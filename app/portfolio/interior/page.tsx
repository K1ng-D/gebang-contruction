...
// ===== Data (khusus Interior) =====
const highlights = [
  { icon: FiShield, title: "Bergaransi", desc: "Garansi pengerjaan & finishing sesuai perjanjian." },
  { icon: FiClock, title: "On Schedule", desc: "Timeline jelas, progress mingguan terdokumentasi." },
  { icon: FiTool, title: "Material Terpilih", desc: "Multiple opsi: HPL/duco/veneer, hardware soft-close." },
  { icon: FiMapPin, title: "Solo Raya", desc: "Solo, Sukoharjo, Karanganyar, Boyolali & sekitar." },
];

const scopes = [
  "Desain & 3D moodboard (style: minimalis, modern, skandinavia, japandi, industrial)",
  "Kitchen set built-in (modul upper/lower, island, top table HPL/solid surface/granite)",
  "Wardrobe & kabinet TV/buffet; rak display & storage tersembunyi",
  "Partisi & panel dinding (WPC, fluted panel, wainscoting, wall moulding)",
  "Meja kerja & workspace ergonomis (cable management, lighting task)",
  "Pintu & kusen, list profil, ambalan, backdrop headboard",
  "Lantai: vinyl/SPC/laminate, karpet tile, leveling & skirting",
  "Pengecatan interior, wallpaper & wallcover, coating anti-jamur",
  "Pencahayaan: cove/hidden LED, downlight/spot, ambient & accent",
  "Elektrikal ringan & manajemen kabel (concealed, neat finish)",
];

// Testimoni
type Testi = { name: string; role: string; comment: string; rating: number };
const testimonials: Testi[] = [
  { name: "Ibu Rina", role: "Apartemen – Solo", comment: "Kitchen set presisi, engsel soft-close, hasil duco halus.", rating: 5 },
  { name: "Bpk. Fajar", role: "Rumah – Colomadu", comment: "Backdrop TV & panel fluted rapi. Mood lighting bikin cozy.", rating: 5 },
  { name: "Ibu Maya", role: "Ruko – Kartasura", comment: "Tata ruang efisien, storage maksimal. Proses sesuai jadwal.", rating: 5 },
];

// Proyek Interior
const projectTeaser = [
  { id: "INT-001", title: "Kitchen Set HPL + LED Strip", location: "Colomadu", year: 2025, cover: project1.src, tags: ["Kitchen", "HPL"] },
  { id: "INT-002", title: "Backdrop TV + Fluted Panel", location: "Solo", year: 2024, cover: project2.src, tags: ["Panel", "Lighting"] },
  { id: "INT-003", title: "Wardrobe Sliding + Cermin", location: "Kartasura", year: 2024, cover: project3.src, tags: ["Wardrobe", "Built-in"] },
  { id: "INT-004", title: "Workspace Minimalis + Wainscoting", location: "Sukoharjo", year: 2025, cover: project4.src, tags: ["Workspace", "Panel"] },
];
...
export default function InteriorPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-blue-50 to-white text-slate-800">
      <Head>
        <title>Interior | Gebang Construction</title>
        <meta
          name="description"
          content="Jasa interior: kitchen set, wardrobe, backdrop TV, panel dinding, flooring, dan lighting. Desain 3D hingga eksekusi rapi, bergaransi."
        />
      </Head>
          ...
          <div className="grid lg:grid-cols-2 gap-10 py-16 lg:py-24 items-center">
            <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} className="space-y-6">
              <motion.h1 variants={fadeUp()} className="text-4xl/tight md:text-5xl/tight font-bold tracking-tight">
                Jasa <span className="bg-gradient-to-r from-amber-400 to-amber-300 bg-clip-text text-transparent">Interior</span> & Custom Furniture
              </motion.h1>
              <motion.p variants={fadeUp(0.1)} className="text-blue-100 max-w-xl">
                Desain fungsional & estetis—dari 3D moodboard sampai instalasi.
                Finishing rapi, storage maksimal, dan lighting nyaman.
              </motion.p>
            ...
            <motion.div className="relative" variants={prefersReducedMotion ? undefined : float} initial="initial" animate="animate">
              <div className="aspect-[4/3] rounded-3xl bg-cover bg-center shadow-2xl relative overflow-hidden">
                ...
                <img src={heroImage.src} alt="Interior – Gebang Construction" className="w-full h-full object-cover" />
              </div>
      ...
      <section id="keunggulan" className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Spesialis interior built-in: kombinasi estetika, fungsi, dan durability yang seimbang.</p>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Bisa paket lengkap (desain → produksi → instalasi) atau sebagian sesuai kebutuhan.</p>
        </div>
      ...
      <section id="proyek" className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12 flex items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-blue-900">Proyek Interior Terbaru</h2>
            <p className="text-slate-600 mt-2 max-w-2xl">Cuplikan beberapa pekerjaan interior built-in terakhir kami.</p>
          </div>
          <Link
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Finishing halus, storage efisien, dan pencahayaan yang nyaman jadi standar kami.</p>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Kalau belum ada gambar, kami bantu dari nol sampai instalasi.</p>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
          {[
            { q: "Apa bedanya HPL, duco, dan veneer?", a: "HPL ekonomis & tahan gores; duco halus & fleksibel warna; veneer serat kayu natural. Kami sesuaikan budget & style." },
            { q: "Apakah ada desain 3D sebelum produksi?", a: "Ya. Kami siapkan layout, moodboard, dan 3D untuk persetujuan sebelum produksi." },
            { q: "Berapa lama pengerjaan kitchen set?", a: "Rata-rata 10–21 hari kerja (desain → produksi workshop → instalasi on-site)." },
            { q: "Skema pembayaran?", a: "Termin progres (DP, produksi, instalasi). Semua transparan & terdokumentasi." },
          ].map((f, idx) => {
      ...
      <section id="kontak" className="relative mx-auto max-w-7xl px-4 pb-16">
        <div className="rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-900 to-blue-700 p-8 md:p-12 shadow-sm overflow-hidden text-white">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <h3 className="text-2xl md:text-3xl font-bold tracking-tight">Konsultasi Interior & Custom Furniture</h3>
              <p className="text-blue-100 mt-2 max-w-prose">
                Kirim ukuran ruang & referensi style—kami bantu desain 3D, pilihan material, dan RAB.
              </p>
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                <a
                  href="mailto:hello@gebangconstruction.id?subject=Konsultasi%20Interior"
            ...
            <motion.div className="aspect-[4/3] rounded-2xl bg-cover bg-center shadow-lg overflow-hidden" variants={prefersReducedMotion ? undefined : float} initial="initial" animate="animate">
              <img src={ctaImage.src} alt="Konsultasi Interior" className="w-full h-full object-cover" />
            </motion.div>
          </div>
        </div>
      </section>
