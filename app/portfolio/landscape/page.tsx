...
// ===== Data (khusus Landscape) =====
const highlights = [
  { icon: FiShield, title: "Bergaransi", desc: "Garansi penanaman & instalasi sistem sesuai perjanjian." },
  { icon: FiClock, title: "On Schedule", desc: "Pekerjaan bertahap: ground work → tanam → finishing." },
  { icon: FiTool, title: "Material Terpilih", desc: "Media tanam curated, batu alam, decking, & lampu outdoor IP65." },
  { icon: FiMapPin, title: "Solo Raya", desc: "Solo, Sukoharjo, Karanganyar, Boyolali & sekitar." },
];

const scopes = [
  "Desain taman (siteplan, moodboard species, layout hardscape/softscape)",
  "Ground work: pengolahan tanah, pemadatan, kontur, & rumput gajah mini/jepang",
  "Hardscape: batu sikat, koral sikat, stepping stone, paving, decking kayu/WPC",
  "Softscape: pohon peneduh, semak, groundcover, vertical garden",
  "Water feature: kolam minimalis, air mancur, biofilter & pompa",
  "Irigasi tetes & sprinkler otomatis (timer, pressure regulator)",
  "Drainase & sumur resapan (pencegah becek/genangan)",
  "Outdoor lighting: bollard, spike, wall washer, strip IP65",
  "Gazebo/pergola, bangku taman, & border planter",
  "Maintenance: pemangkasan, pemupukan, pengendalian hama",
];

// Testimoni
type Testi = { name: string; role: string; comment: string; rating: number };
const testimonials: Testi[] = [
  { name: "Ibu Nia", role: "Rumah – Solo Baru", comment: "Layout taman lapang, lighting malam hari cakep. Rumput tumbuh rata.", rating: 5 },
  { name: "Bpk. Yoga", role: "Ruko – Colomadu", comment: "Vertical garden segar, irigasi otomatis praktis.", rating: 5 },
  { name: "Ibu Lilis", role: "Kantor – Kartasura", comment: "Kolam minimalis dengan biofilter jernih & low-maintenance.", rating: 5 },
];

// Proyek Landscape
const projectTeaser = [
  { id: "LND-001", title: "Taman Minimalis + Lighting", location: "Solo", year: 2025, cover: project1.src, tags: ["Lighting", "Minimalis"] },
  { id: "LND-002", title: "Decking WPC + Planter Box", location: "Colomadu", year: 2024, cover: project2.src, tags: ["Decking", "Hardscape"] },
  { id: "LND-003", title: "Vertical Garden + Irigasi Tetes", location: "Kartasura", year: 2024, cover: project3.src, tags: ["Vertical", "Irigasi"] },
  { id: "LND-004", title: "Kolam Air Mancur + Batu Alam", location: "Sukoharjo", year: 2025, cover: project4.src, tags: ["Water Feature", "Batu Alam"] },
];
...
export default function LandscapePage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-blue-50 to-white text-slate-800">
      <Head>
        <title>Landscape | Gebang Construction</title>
        <meta
          name="description"
          content="Jasa landscape: desain taman, hardscape–softscape, irigasi otomatis, lighting outdoor, kolam, dan maintenance. On schedule & bergaransi."
        />
      </Head>
          ...
          <div className="grid lg:grid-cols-2 gap-10 py-16 lg:py-24 items-center">
            <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} className="space-y-6">
              <motion.h1 variants={fadeUp()} className="text-4xl/tight md:text-5xl/tight font-bold tracking-tight">
                Jasa <span className="bg-gradient-to-r from-amber-400 to-amber-300 bg-clip-text text-transparent">Landscape</span> & Taman
              </motion.h1>
              <motion.p variants={fadeUp(0.1)} className="text-blue-100 max-w-xl">
                Taman asri & fungsional—kombinasi hardscape, softscape, irigasi, dan lighting. Dari desain hingga maintenance.
              </motion.p>
              ...
              <motion.ul variants={fadeUp(0.3)} className="flex items-center gap-4 text-blue-100 text-sm">
                ...
                <li className="inline-flex items-center gap-2"><FiCheckCircle className="text-amber-400" /> Perawatan Mudah</li>
              </motion.ul>
            </motion.div>
            ...
            <motion.div className="relative" variants={prefersReducedMotion ? undefined : float} initial="initial" animate="animate">
              <div className="aspect-[4/3] rounded-3xl bg-cover bg-center shadow-2xl relative overflow-hidden">
                ...
                <img src={heroImage.src} alt="Landscape – Gebang Construction" className="w-full h-full object-cover" />
              </div>
      ...
      <section id="keunggulan" className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Desain menyatu dengan arsitektur, fungsi jelas, minim perawatan, dan tahan cuaca.</p>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Ambil paket lengkap atau bagian tertentu—fleksibel sesuai kebutuhan.</p>
        </div>
      ...
      <section id="proyek" className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12 flex items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-blue-900">Proyek Landscape Terbaru</h2>
            <p className="text-slate-600 mt-2 max-w-2xl">Cuplikan taman & hardscape terbaru—dengan lighting & irigasi otomatis.</p>
          </div>
          <Link
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Drainase rapi, irigasi hemat air, dan lighting dramatis jadi standar kami.</p>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">Belum ada konsep? Tenang—kami bantu dari survey sampai perawatan.</p>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
          {[
            { q: "Apa perbedaan rumput gajah mini vs jepang?", a: "Gajah mini: perawatan mudah & hemat; Jepang: lebih rapat & halus namun butuh perawatan lebih intens." },
            { q: "Butuh irigasi otomatis?", a: "Disarankan untuk area luas/vertikal. Timer & drip line menjaga kelembapan konsisten dan hemat air." },
            { q: "Bagaimana mencegah genangan?", a: "Rancang kontur, sediakan saluran, pasang batu koral di area resapan, dan buat sump kecil bila perlu." },
            { q: "Perawatan pasca tanam?", a: "Penyiraman rutin, pemupukan awal 2–4 minggu, dan pengendalian hama ringan. Kami sediakan paket maintenance." },
          ].map((f, idx) => {
      ...
      <section id="kontak" className="relative mx-auto max-w-7xl px-4 pb-16">
        <div className="rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-900 to-blue-700 p-8 md:p-12 shadow-sm overflow-hidden text-white">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <h3 className="text-2xl md:text-3xl font-bold tracking-tight">Konsultasi Desain Taman Gratis</h3>
              <p className="text-blue-100 mt-2 max-w-prose">
                Kirim ukuran lahan, foto lokasi, & referensi style—kami bantu desain, RAB, irigasi, dan jadwal pengerjaan.
              </p>
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                <a
                  href="mailto:hello@gebangconstruction.id?subject=Konsultasi%20Landscape"
            ...
            <motion.div className="aspect-[4/3] rounded-2xl bg-cover bg-center shadow-lg overflow-hidden" variants={prefersReducedMotion ? undefined : float} initial="initial" animate="animate">
              <img src={ctaImage.src} alt="Konsultasi Landscape" className="w-full h-full object-cover" />
            </motion.div>
          </div>
        </div>
      </section>
